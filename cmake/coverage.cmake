...
# Lips, lisp shell.
#
# Copyright 2021-2022, 2024 Krister Joas
#
...
if("${CMAKE_C_COMPILER_ID}" MATCHES "(Apple)?[Cc]lang" OR "${CMAKE_CXX_COMPILER_ID}" MATCHES
  ...
  add_custom_target(
    ...
    COMMAND ${LLVM_PROFDATA} merge
            -sparse ${CMAKE_CURRENT_BINARY_DIR}/${TARGET_NAME}.profraw
  ...
  add_custom_target(
    ...
    COMMAND ${LLVM_COV} report $<TARGET_FILE:${TARGET_NAME}>
            ...
            --format="text" --ignore-filename-regex=exit.hh --ignore-filename-regex='.*.test.cc'
...
elseif(CMAKE_COMPILER_IS_GNUCXX)
  ...
  add_custom_target(ccov-preprocessing
    COMMAND GCOV_PROFILE_DIR=${CMAKE_CURRENT_BINARY_DIR}/ $<TARGET_FILE:${TARGET_NAME}>
            --load ${CMAKE_CURRENT_SOURCE_DIR}/lisp/test.lisp
            --loadpath ${CMAKE_CURRENT_SOURCE_DIR}
            --loadpath ${CMAKE_CURRENT_BINARY_DIR} "$ENV{LIPS_TEST_TAGS}"
    DEPENDS ${TARGET_NAME})
  add_custom_target(lcov-capture
    ...
    COMMAND lcov --capture --exclude='*test.cc' --directory ${CMAKE_CURRENT_BINARY_DIR}/src/lisp
            -o ${CMAKE_CURRENT_BINARY_DIR}/output.info DEPENDS ccov-preprocessing)
  add_custom_target(
    lcov-remove
    COMMAND rm -f ${CMAKE_CURRENT_BINARY_DIR}/coverage.info
    COMMAND lcov --remove ${CMAKE_CURRENT_BINARY_DIR}/output.info '/usr/include/*'
            '${CMAKE_CURRENT_BINARY_DIR}/_deps/*' '${CMAKE_CURRENT_BINARY_DIR}/install/*' -o
            ${CMAKE_CURRENT_BINARY_DIR}/coverage.info
    DEPENDS lcov-capture)
  add_custom_target(lcov-genhtml
    COMMAND rm -rf ${CMAKE_CURRENT_BINARY_DIR}/html
    COMMAND genhtml ${CMAKE_CURRENT_BINARY_DIR}/coverage.info -o ${CMAKE_CURRENT_BINARY_DIR}/html
    DEPENDS lcov-remove)
